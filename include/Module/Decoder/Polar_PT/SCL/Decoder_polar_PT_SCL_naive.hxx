...
Decoder_polar_PT_SCL_naive<B, R, F, G>::Decoder_polar_PT_SCL_naive(
  const int& K,
  const int& N,
  const int& L,
  ...
  , L(L)
  , polar_trees(L, tools::Binary_tree_metric<Contents_PT_SCL<B, R>, R>(this->m + 1, metric_init))
{
    /*std::cout << "Inside the SCL naive decoder: " << __FILE__ << std::endl;*/
    const std::string name = "Decoder_polar_PT_SCL_naive";
    this->set_name(name);
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
Decoder_polar_PT_SCL_naive<B, R, F, G>*
Decoder_polar_PT_SCL_naive<B, R, F, G>::clone() const
{
    auto m = new Decoder_polar_PT_SCL_naive(*this);
    m->deep_copy(*this);
    return m;
}
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
void
Decoder_polar_PT_SCL_naive<B, R, F, G>::deep_copy(const Decoder_polar_PT_SCL_naive<B, R, F, G>& m)
{
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
void
Decoder_polar_PT_SCL_naive<B, R, F, G>::recursive_deep_copy(const tools::Binary_node<Contents_PT_SCL<B, R>>* nref,
                                                            tools::Binary_node<Contents_PT_SCL<B, R>>* nclone)
{
    auto cref = nref->get_contents();
    auto cclone = new Contents_PT_SCL<B, R>(*cref);
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
void
Decoder_polar_PT_SCL_naive<B, R, F, G>::set_pretransform(const std::map<uint32_t, std::vector<uint32_t>>& pretransform)
{
}

template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
void
Decoder_polar_PT_SCL_naive<B, R, F, G>::set_frozen_bits(const std::vector<bool>& fb)
{
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
void
Decoder_polar_PT_SCL_naive<B, R, F, G>::set_dynamic_frozen_bits(const std::vector<bool>& dfb)
{
...
Decoder_polar_PT_SCL_naive<B, R, F, G>::get_pretransform() const
{
...
Decoder_polar_PT_SCL_naive<B, R, F, G>::get_frozen_bits() const
{
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
const std::vector<bool>&
Decoder_polar_PT_SCL_naive<B, R, F, G>::get_dynamic_frozen_bits() const
{
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
void
Decoder_polar_PT_SCL_naive<B, R, F, G>::_load(const R* Y_N)
{
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
void
Decoder_polar_PT_SCL_naive<B, R, F, G>::_decode(const size_t frame_id)
{
    std::set<int> last_active_paths;
    int cur_path;

    using PtType = std::pair<B, B>;
    ...
    std::vector<std::tuple<int, B, R, B, int, PtType>> metrics_vec;
    ...
    for (auto leaf_index = 0; leaf_index < this->N; leaf_index++)
    {
        ...
        if (leaves_array[0][leaf_index]->get_c()->is_frozen_bit)
        {
            ...
            for (auto path : active_paths)
            {
                ...
                if (dynamic_frozen_bits[leaf_index])
                {
                    B res = preTransform1Bit((B)0, leaf_index, path);
                    cur_leaf->get_c()->s[0] = res ? spu::tools::bit_init<B>() : 0;
                }
        ...
        else
        {
            ...
            for (auto path : active_paths)
            {
                ...
                u0 = u0 ? spu::tools::bit_init<B>() : 0;
                u1 = u1 ? spu::tools::bit_init<B>() : 0;
                // ConvType curConv = std::make_tuple(u0Pair, u1Pair);
                PtType curtype = std::make_pair(u0, u1);
                ...
                metrics_vec.push_back(std::make_tuple(path, u0, phi0, (B)0, ci, curtype));
                metrics_vec.push_back(std::make_tuple(path, u1, phi1, (B)1, ci, curtype));
            ...
            if (active_paths.size() <= (unsigned)(L / 2))
            {
                ...
                {
                    auto cur_it =
                      std::find_if(metrics_vec.begin(),
                                   metrics_vec.end(),
                                   [path](std::tuple<int, B, R, B, int, PtType> x) { return std::get<0>(x) == path; });
            ...
            else
            {
                ...
                std::sort(metrics_vec.begin(),
                          metrics_vec.end(),
                          [](std::tuple<int, B, R, B, int, PtType> x, std::tuple<int, B, R, B, int, PtType> y)
                ...
                for (auto it = metrics_vec.begin() + metrics_vec.size() / 2; it != metrics_vec.end(); ++it)
                {
                    ...
                    auto it_double = std::find_if(it + 1,
                                                  metrics_vec.end(),
                                                  [cur_path](std::tuple<int, B, R, B, int, PtType> x)
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
int
Decoder_polar_PT_SCL_naive<B, R, F, G>::_decode_siho(const R* Y_N, B* V_K, const size_t frame_id)
{
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
int
Decoder_polar_PT_SCL_naive<B, R, F, G>::_decode_siho_cw(const R* Y_N, B* V_N, const size_t frame_id)
{
    //	auto t_load = std::chrono::steady_clock::now(); //
    //-----------------------------------------------------------
    // LOAD
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
void
Decoder_polar_PT_SCL_naive<B, R, F, G>::_store(B* V, bool coded) const
{
    auto* root = (tools::Binary_node<Contents_PT_SCL<B, R>>*)this->polar_trees[*active_paths.begin()].get_root();
    if (!coded)
    {
        auto k = 0;
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
void
Decoder_polar_PT_SCL_naive<B, R, F, G>::recursive_compute_llr(tools::Binary_node<Contents_PT_SCL<B, R>>* node_cur,
                                                              int depth)
{
    if (depth != 0) recursive_compute_llr(node_cur->get_father(), --depth);

    if (node_cur->is_left()) this->apply_f((tools::Binary_node<Contents_PT_SCL<B, R>>*)node_cur->get_father());

    if (node_cur->is_right()) this->apply_g((tools::Binary_node<Contents_PT_SCL<B, R>>*)node_cur->get_father());
}

template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
void
Decoder_polar_PT_SCL_naive<B, R, F, G>::propagate_sums(const tools::Binary_node<Contents_PT_SCL<B, R>>* node_cur)
{
    if (!node_cur->is_leaf()) this->compute_sums((tools::Binary_node<Contents_PT_SCL<B, R>>*)node_cur);
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
void
Decoder_polar_PT_SCL_naive<B, R, F, G>::recursive_duplicate_tree_llr(tools::Binary_node<Contents_PT_SCL<B, R>>* node_a,
                                                                     tools::Binary_node<Contents_PT_SCL<B, R>>* node_b)
{
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
void
Decoder_polar_PT_SCL_naive<B, R, F, G>::recursive_duplicate_tree_sums(
  tools::Binary_node<Contents_PT_SCL<B, R>>* node_a,
  tools::Binary_node<Contents_PT_SCL<B, R>>* node_b,
  tools::Binary_node<Contents_PT_SCL<B, R>>* node_caller)
{
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
void
Decoder_polar_PT_SCL_naive<B, R, F, G>::duplicate_path(int path, int leaf_index, B mkz, B mko)
{
    std::vector<tools::Binary_node<Contents_PT_SCL<B, R>>*> path_leaves, newpath_leaves;
    int newpath = 0;
    ...
    leaves_array[newpath][leaf_index]->get_c()->s[0] = mko ? spu::tools::bit_init<B>() : 0;
    ...
    leaves_array[path][leaf_index]->get_c()->s[0] = mkz ? spu::tools::bit_init<B>() : 0;
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
void
Decoder_polar_PT_SCL_naive<B, R, F, G>::select_best_path(const size_t /*frame*/)
{
    int best_path = 0;
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
void
Decoder_polar_PT_SCL_naive<B, R, F, G>::recursive_allocate_nodes_contents(
  tools::Binary_node<Contents_PT_SCL<B, R>>* node_curr,
  const int vector_size)
{
    if (node_curr != nullptr)
    {
        node_curr->set_contents(new Contents_PT_SCL<B, R>(vector_size));
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
void
Decoder_polar_PT_SCL_naive<B, R, F, G>::recursive_initialize_frozen_bits(
  const tools::Binary_node<Contents_PT_SCL<B, R>>* node_curr,
  const std::vector<bool>& frozen_bits)
{
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
void
Decoder_polar_PT_SCL_naive<B, R, F, G>::recursive_store(const tools::Binary_node<Contents_PT_SCL<B, R>>* node_curr,
                                                        B* V_K,
                                                        int& k) const
{
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
void
Decoder_polar_PT_SCL_naive<B, R, F, G>::recursive_deallocate_nodes_contents(
  tools::Binary_node<Contents_PT_SCL<B, R>>* node_curr)
{
    if (node_curr != nullptr)
    {
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
void
Decoder_polar_PT_SCL_naive<B, R, F, G>::apply_f(const tools::Binary_node<Contents_PT_SCL<B, R>>* node_curr)
{
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
void
Decoder_polar_PT_SCL_naive<B, R, F, G>::apply_g(const tools::Binary_node<Contents_PT_SCL<B, R>>* node_curr)
{
    const auto size = (int)node_curr->get_c()->lambda.size();
    const auto size_2 = size / 2;
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
void
Decoder_polar_PT_SCL_naive<B, R, F, G>::compute_sums(const tools::Binary_node<Contents_PT_SCL<B, R>>* node_curr)
{
    const auto size = (int)node_curr->get_c()->lambda.size();
    const auto size_2 = size / 2;
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G>
B
Decoder_polar_PT_SCL_naive<B, R, F, G>::preTransform1Bit(const B cbit, const int index, const int path)
{
    B temp_bit = (B)cbit;
    if (this->pre_transform.find(index) != this->pre_transform.end())
    {
        // std::cout << "Index: " << index << ",";
        for (const auto& it : this->pre_transform[index])
        {
            int inf_index = info_bits_loc[it];
            // std::cout << inf_index << ",";
            if (inf_index < index)
            {
                // std::cout << info_bits_loc[it] << ",";

                temp_bit = temp_bit ^ this->leaves_array[path][inf_index]->get_c()->v[0];
            }
        }
        // std::cout << std::endl;
    }
